...
# Drop the already-terminated coax into the open edge; no threading tiny plugs.
AX=P['antenna_cable']['exit_x'];AW=P['antenna_cable']['exit_width_mm'];AD=P['antenna_cable']['exit_depth_mm']
boolean(base,cube('coax drop-in exit',(AX-AW/2,IY1-1,Z-AD),(AX+AW/2,Y1+1,Z+1)))
# Broad, rounded tie saddle attached to the rear wall. A 2.5 mm nylon tie
# passes below its 3 mm bridge; tighten only enough to retain the cable.
saddle=round_rect('coax tie saddle',AX-5,AX+5,IY1-7,IY1+0.1,1,27,33)
boolean(saddle,cube('tie slot',(AX-1.75,IY1-8,28),(AX+1.75,IY1+1,30)))
boolean(base,saddle,'UNION')
...
boolean(lid,cube('coax lip relief',(AX-AW/2,IY1-C-LW-1,Z-AD),(AX+AW/2,Y1+1,Z)))
for x,y in P['lid_mounts']:
...
assert all(abs(radio_bounds[i][j]-[[64,82],[-14.9,-0.6]][i][j])<0.03
...
validation={'pcb_sha256':G['pcb_sha256'],'revision':P['revision'],'units':'mm',
    ...
    'antenna_cable_exit':P['antenna_cable'],
    'limitations':['Printed fit, nut/screw sizes and cable hood must be checked physically.',
      'ANT1 mounts outside the casing on suitable plastic. Fit one small nylon tie at the saddle, leave a slack loop at U2, smooth the exit and physically verify no pinching or connector load.',
