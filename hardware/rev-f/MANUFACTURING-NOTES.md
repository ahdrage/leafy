# Rev F — partial factory assembly, quotation/prototype files

**100 × 100 mm. Use Rev F files only. Fit exactly U1, U2, U4 and L1 on the top.** All other component positions are intentionally left empty for the customer to hand solder. This is an incomplete engineering prototype; do not energize or claim functional test of the delivered partially assembled circuit.
...
The hand-parts value table is printed on the back. J1 is now NorComp 182-009-113R561 with fitted female screwlocks. Its plated-hole pattern is unchanged from Rev E. The printed outline is clipped at the PCB edge; allow the connector and screwlocks to overhang freely. The complete native project shows every final component for circuit and mechanical review. The generic DB9 3D model is approximate and is not an enclosure specification.
