# Rev F — hand-assembly guide

The factory fits **U1, U2, U4 and L1**. Fit the remaining **37 parts** using `BOM-HAND.csv` and the assembly map. The parts-value table is printed on the back to leave room between the top-side components. Every component mounts on the top. Only through-hole solder joints are made from the underside.
...
| U3 | TCAN3404DRQ1 SOIC-8; pin 5 is SHDN and is grounded on Rev F. Do not use the old TCAN3403 part here. Align pin 1 with the board's pin-1 marker; inspect all eight leads for bridges. |
