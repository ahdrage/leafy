"""Export final Rev D. Factory CSVs and paste contain exactly four parts."""
from pathlib import Path
import os,subprocess,csv,collections,json,hashlib,xml.etree.ElementTree as ET,html
import pcbnew as p
import cairosvg
H=Path(__file__).resolve().parent;E=H/'exports';B=H/'leaf-heat-v4.kicad_pcb'
...
for layer,name in [('F.Cu,F.Silkscreen,Edge.Cuts','top'),('In1.Cu,Edge.Cuts','ground'),('In2.Cu,Edge.Cuts','power'),('B.Cu,Edge.Cuts','bottom'),('F.Fab,Edge.Cuts','placement'),('F.Paste,Edge.Cuts','factory-paste')]:
...
run('sch','export','svg','-o',E/'schematic',H/'leaf-heat-v4.kicad_sch')
cairosvg.svg2png(url=str(E/'schematic/leaf-heat-v4.svg'),write_to=str(E/'schematic.png'),output_width=4000,background_color='white')
run('sch','export','pdf','-o',E/'schematic.pdf',H/'leaf-heat-v4.kicad_sch')
...
for q in rows:
    ...
    assert abs(float(q['PosY'])-(150-p.ToMM(f.GetPosition().y)))<.00001
...
svg=['<svg xmlns="http://www.w3.org/2000/svg" width="1900" height="2050" viewBox="-5 -18 160 173">',
     '<rect x="-5" y="-18" width="160" height="173" fill="#f8fafc"/>',
     '<text x="0" y="-11" font-family="Arial" font-size="3.7" fill="#0f172a">LEAF HEAT REV D — 150 × 150 mm</text>',
     '<text x="0" y="-5" font-family="Arial" font-size="2.5" fill="#9a3412">Orange: factory fits U1 / U2 / U4 / L1. Remaining 37 parts: hand solder.</text>',
     '<rect x="0" y="0" width="150" height="150" rx="0" fill="#e2e8f0" stroke="#475569" stroke-width=".25"/>']
...
for i,(text,x,y) in enumerate([('POWER',4,41),('Wi-Fi',110,38),('CAR / CAN',4,108),('RESET / BOOT',103,107),('UART',128,126)]):
...
# Targeted circuit crops are easier to read than zooming the complete A3 page.
for name,box in [('power-schematic','15 28 237 112'),('uart-schematic','15 139 221 91'),('wifi-schematic','252 18 149 126'),('can-schematic','252 146 149 99')]:
    root=ET.parse(E/'schematic/leaf-heat-v4.svg').getroot();root.set('viewBox',box);root.set('width',box.split()[2]+'mm');root.set('height',box.split()[3]+'mm')
    cairosvg.svg2png(bytestring=ET.tostring(root),write_to=str(E/(name+'.png')),output_width=2200,background_color='white')
report={'component_count':41,'factory_count':4,'hand_count':37,'factory_refs':sorted(paste),'hand_mpns':len(groups(hand)),'unique_mpns':len(groups(parts)),
 'placement_origin':'lower-left (0,150 mm in KiCad), X right / Y up','factory_placement_rows':4,'full_placement_rows':41,
