...
import pcbnew as p
H=Path(__file__).resolve().parent;E=H/'exports';C=H.parent/'rev-d'
checks=[]
def check(ok,why):
...
b=p.LoadBoard(str(H/'leaf-heat-v5.kicad_pcb'));fps={f.GetReference():f for f in b.GetFootprints()}
...
check(all(hashlib.sha256((C/n).read_bytes()).hexdigest()==sha for n,sha in baseline.items()),'Rev D native project is preserved byte-for-byte')
...
root,pins,nets=netlist(E/'leaf-heat-v5.net.xml');_,cpins,cnets=netlist(C/'exports/leaf-heat-v4.net.xml')
check(all(pins[k]==n for k,n in cpins.items() if k[0] in meta),'Every original schematic pin retains its Rev D net')
for q in parts:
    ref=q['ref'];f=fps[ref]
    ...
    if ref.startswith('R'):
        check(q['mpn']==old[ref]['mpn'] and q['value']==old[ref]['value'],f'{ref} is the same 1% resistance in 1206')
...
for ref in meta:
    expected_mpn = 'B1100-13-F' if ref == 'D1' else old[ref]['mpn']
    check(meta[ref]['mpn']==expected_mpn,f'{ref} exact component retained or explicitly corrected by D1 purchasing erratum')
...
check(len(outline)==4 and corners=={(0,0),(100,0),(100,100),(0,100)},'Board outline is exactly 100 x 100 mm')
check(xy(b.GetDesignSettings().GetAuxOrigin())==(0,100),'Manufacturing origin is the lower-left corner')
...
miss=[]
# Test the interior of the 59..87 mm rule area. Polygon edge ownership
# at exactly x=87 is numerically ambiguous in HitTestFilledArea.
for x in [59.1,*range(60,87),86.9]:
    for y in [.4,.6,.8]:
...
settings=json.loads((H/'leaf-heat-v5.kicad_pro').read_text());check(not settings['board']['design_settings']['drc_exclusions'],'No individual DRC exclusions')
...
# Quantify actual spacing improvements, not just a smaller board outline.
spacing=[]
for r,t in [('C5','C6'),('C6','C7'),('R1','R2'),('R3','C8'),('R5','R6'),('R18','R19'),('SW1','SW2'),('R9','D6')]:
    was=math.dist(old[r]['xy'],old[t]['xy']);now_distance=math.dist(xy(fps[r].GetPosition()),xy(fps[t].GetPosition()))
    check(now_distance>was, r+'/'+t+' centre spacing increased versus Rev D')
    spacing.append({'pair':r+'/'+t,'Rev_D_mm':was,'Rev_E_mm':now_distance})
for r in ['U1','C1','C2','C3','C4','L1']:
    check(xy(fps[r].GetPosition())==tuple(old[r]['xy']) and abs(fps[r].GetOrientationDegrees()-old[r]['rotation'])<1e-6,r+' critical power placement retained')
report={'revision':'E','board_mm':[100,100],'checks_passed':len(checks),'factory_refs':sorted(factory),'factory_count':4,'hand_count':37,'bare_testpoints':7,'mounting_holes':4,
 'spacing_comparison':spacing,'component_count':41,'unique_mpns':len({q['mpn'] for q in parts}),'preserved_critical_copper_items':len(bt),
 ...
 'board_sha256':hashlib.sha256((H/'leaf-heat-v5.kicad_pcb').read_bytes()).hexdigest(),
 'schematic_sha256':hashlib.sha256((H/'leaf-heat-v5.kicad_sch').read_bytes()).hexdigest(),
