...
b=p.LoadBoard(str(H/'leaf-heat-v5.kicad_pcb'))
shutil.copy2(H/'leaf-heat-v5.kicad_pcb',E/'before-router.kicad_pcb')
assert p.ExportSpecctraDSN(b,str(E/'leaf-heat-v5.dsn'))
s=(E/'leaf-heat-v5.dsn').read_text()
def replace(m):
    t=m.group()
...
(E/'leaf-heat-v5-routing.dsn').write_text(s)
