...
Rev E arrives with only U1, U2, U4 and L1 fitted. Complete the remaining 37 components and the inspection in HAND-ASSEMBLY.md before connecting power.

# Programming Rev E

Rev E uses the ESP32-C3's built-in **UART0 bootloader**. The programmer is a reusable external accessory; there is no USB socket on this PCB. The firmware has not yet been written, so the instructions below define the hardware setup and recovery procedure, not a claim that heater software is ready.
...
3. A female DB9 breakout or a verified bench lead mating with J1, connecting only **pin 9 to +12 V and pin 3 to supply ground**. J1 and J3 are hand-fitted in Rev E and must be soldered and inspected before bench programming. The breakout/bench supply are separate accessories, not items in the PCB BOM.
