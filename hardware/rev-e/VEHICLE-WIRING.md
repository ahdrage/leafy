# Rev E vehicle wiring and firmware interface

This guide applies to the **100 × 100 mm custom Rev E board**. The earlier development-board plan has different GPIO assignments. No runnable heater firmware is supplied. Complete hand assembly and bench tests before vehicle connection.
...
| Vehicle OBD-II contact | Cable DB9 / board J1 contact | Rev E function |
| --- | --- | --- |
...
The intended profile is the **2013–2015 EV-CAN remote-climate implementation**. Registration in 2015 and the registration document's ZE0 label alone do not establish the TCU generation. Record the vehicle build/model year and fitted TCU before enabling transmissions.
...
**Rev E cannot select the alternate bus in software.** It has one transceiver connected to J1 pins 2/7; pins 4/5 go nowhere. A car requiring the later profile needs a reviewed cable remapping or hardware revision as well as different firmware commands. This PCB also lacks the separate activation output used by the earliest cars.
...
| Protected-input measurement | 0 / ADC1_CH0 | 18 | R7/R8/C11 battery divider |
