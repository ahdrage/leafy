"""Export final Rev G. Factory CSVs and paste contain exactly six parts."""
from pathlib import Path
import os,subprocess,csv,collections,json,hashlib,xml.etree.ElementTree as ET,html
import pcbnew as p
import cairosvg
H=Path(__file__).resolve().parent;E=H/'exports';B=H/'leaf-heat-v7.kicad_pcb'
...
run('sch','export','svg','-o',E/'schematic',H/'leaf-heat-v7.kicad_sch')
cairosvg.svg2png(url=str(E/'schematic/leaf-heat-v7.svg'),write_to=str(E/'schematic.png'),output_width=4000,background_color='white')
run('sch','export','pdf','-o',E/'schematic.pdf',H/'leaf-heat-v7.kicad_sch')
...
(E/'fabrication/FABRICATION-NOTES.txt').write_bytes((H/'FABRICATION-NOTES.txt').read_bytes())
...
assert {q['ref'] for q in factory}=={'U1','U2','U4','U5','U6','L1'} and len(hand)==49
...
with (E/'BOM-FACTORY-PCBWay.csv').open('w') as f:
    ...
    for i,q in enumerate(factory,1):w.writerow([i,q['ref'],1,q['manufacturer'],q['mpn'],q['value'],q['footprint'],'FACTORY FIT. Only U1 U2 U4 U5 U6 L1 are factory-assembled; all other parts customer-fitted.'])
with (E/'BOM-FACTORY-JLCPCB.csv').open('w') as f:
    ...
    catalog={'LMR36510ADDAR':'C1858393','ESP32-C3-WROOM-02U-N4':'C2926676','USBLC6-2SC6':'C7519','TPS3760A012DYYR':'C5218894','TMUX1511PWR':'C2866750','SRN6045TA-220M':'C2044314'}
    assert {q['mpn'] for q in factory}==set(catalog)
    for q in factory:w.writerow([q['value'],q['ref'],q['footprint'],q['manufacturer'],q['mpn'],catalog[q['mpn']]])
...
assert len(rows)==len(refs)==55 and {q['Ref'] for q in rows}==refs
fr=[q for q in rows if q['Ref'] in {x['ref'] for x in factory}];assert len(fr)==6
...
jlc_rotation_offsets={'U1':270,'U4':270,'U5':270,'U6':270}
# JLC catalogue model zero angles differ from KiCad. These offsets were
# visually checked against native pad 1 and every lead in the live viewer.
for filename,offsets in [('CPL-NATIVE-REFERENCE-ONLY.csv',{}),('CPL-FACTORY-JLCPCB.csv',jlc_rotation_offsets)]:
    with (E/filename).open('w') as f:
        w=csv.writer(f);w.writerow(['Designator','Mid X','Mid Y','Layer','Rotation'])
        for q in fr:w.writerow([q['Ref'],q['PosX'],q['PosY'],'Top',f"{(float(q['Rot'])+offsets.get(q['Ref'],0))%360:.6f}"])
for q in rows:
    f=fps[q['Ref']]
...
assert paste=={'U1','U2','U4','U5','U6','L1'},paste
...
svg=['<svg xmlns="http://www.w3.org/2000/svg" width="1800" height="2010" viewBox="-5 -18 110 123">',
     ...
     '<text x="0" y="-11" font-family="Arial" font-size="2.9" fill="#0f172a">Leafy REV G.5 — 100 × 100 mm</text>',
     '<text x="0" y="-5" font-family="Arial" font-size="1.8" fill="#9a3412">Orange: factory fits U1 / U2 / U4 / U5 / U6 / L1. Remaining 49 parts: hand solder.</text>',
...
report={'component_count':55,'factory_count':6,'hand_count':49,'factory_refs':sorted(paste),'hand_mpns':len(groups(hand)),'unique_mpns':len(groups(parts)),
 'placement_origin':'lower-left (0,100 mm in KiCad), X right / Y up','factory_placement_rows':6,'full_placement_rows':55,
 'stencil_refs':sorted(paste),'bare_test_holes':7,'mounting_holes':4,'board_sha256':hashlib.sha256(B.read_bytes()).hexdigest(),
 'catalog_ids':catalog,'jlc_catalog_rotation_offsets_degrees':jlc_rotation_offsets,'fabrication_files':{f.name:hashlib.sha256(f.read_bytes()).hexdigest() for f in sorted((E/'fabrication').iterdir()) if f.is_file()}}
report['revision']='G.5'
report['thermal_process_note']='FILLED-G5-CHANGE.md: all 18 thermal PTHs epoxy filled and copper capped; normal JLC stencil engineering, no fixed thickness/apertures'
(E/'manufacturing-audit.json').write_text(json.dumps(report,indent=2)+'\n')
print('Verified 6 factory placements, 49 hand parts, and factory-only paste.')
