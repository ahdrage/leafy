...
Rev C uses the ESP32-C3's built-in **UART0 bootloader**. The programmer is a reusable external accessory; there is no USB socket on this PCB. The firmware has not yet been written, so the instructions below define the hardware setup and recovery procedure, not a claim that heater software is ready.
